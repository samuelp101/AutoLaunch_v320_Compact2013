...
xcopy     "%_UDPCLIENTPATH%\Compact2013UDPClient.exe"       "%SG_OUTPUT_ROOT%\oak\target\%_TGTCPU%\%WINCEDEBUG%"  /Y
...
@echo Copying Compact2013UDPClient to FlatRelease Directory
xcopy  "%_UDPCLIENTPATH%\Compact2013UDPClient.exe"  %_FLATRELEASEDIR%  /Y

:DONECOPY
@echo .
@echo Done copying. Now checking.

If NOT EXIST  %SG_OUTPUT_ROOT%\oak\target\%_TGTCPU%\%WINCEDEBUG%\Compact2013UDPClient.exe (
rem		@echo Error Compact2013UDPClient.exe is missing from the Targeted Debug Directory.
	@echo Error Compact2013UDPClient.exe is missing from the Targeted Debug Directory.
	GOTO ABORT
)
